...
- **トピック追跡** — 複数の報道がある topic_id を `/topics/` でタイムライン表示（続報の経過を追跡）
- **収集メトリクス** — 両パイプラインのソース別件数・本文取得率・要約成功率を日次蓄積。異常検知 + index.html バナー表示
- **Issue 自動通知** — ワークフロー失敗と健全性警告を GitHub Issue で通知（警告解消時は自動クローズ）
...
python -m scripts.metrics --claude     # Claude パイプラインのメトリクス
python -m scripts.metrics --check      # 両パイプラインの警告のみ（CI用）
...
| `data/claude_metrics.json` | Claude パイプラインの日次メトリクス |
