---
title: Market Trends
category: ecosystem
subcategory: market-trends
tags:
- claude-code
- cowork
- performance
- pricing
- 新機能
date: '2026-04-03'
updated: '2026-08-15'
sources:
- url: https://qiita.com/kotauchisunsun/items/ab78bb338500b4c71103
  title: Claude Codeの首位陥落。CodexがシェアNo.1へ。 ～データで見る2026年3月のAI Codingの動向まとめ～
  date: '2026-04-03'
- url: https://techcrunch.com/2026/06/25/anthropics-claude-is-winning-over-paid-consumers-a-market-owned-by-chatgpt
  title: Anthropic’s Claude is winning over paid consumers, a market owned by ChatGPT
  date: '2026-06-25'
- url: https://the-decoder.com/investor-pressure-forces-nvidia-to-shrink-its-openai-bet-just-as-anthropics-numbers-defy-bubble-warnings
  title: Investor pressure forces Nvidia to shrink its OpenAI bet just as Anthropic's
    numbers defy bubble warnings
  date: '2026-08-15'
---



# Market Trends

---

## 2026-08-15

### Investor pressure forces Nvidia to shrink its OpenAI bet just as Anthropic's numbers defy bubble warnings

Nvidiaは投資家の圧力により、OpenAIへのデータセンター支援を当初の2500億ドルから1200億ドルに半減させた。一方でAnthropicは四半期で売上が倍増し、Q2に115億ドルを達成。2028年には1900-2000億ドルの売上を見込み、時価総額1兆ドルでの9-10月IPOを計画中。Nvidiaのリスク縮小がAIバブル懸念を示唆する一方、Anthropicの急成長は需要の持続性を示している。

- **ソース**: [The Decoder Claude](https://the-decoder.com/investor-pressure-forces-nvidia-to-shrink-its-openai-bet-just-as-anthropics-numbers-defy-bubble-warnings)
- **重要度**: 8/10
- **タグ**: pricing, performance, 新機能

---

## 2026-06-25

### Anthropic’s Claude is winning over paid consumers, a market owned by ChatGPT

クレジットカード取引分析によると、Anthropic の Claude の有料ユーザー数と収益が2026年1月以降75%増加し、消費者市場で急成長している。DataCamp では「Claude」が最も検索されるキーワードとなり、自己学習する消費者の間では ChatGPT の3倍の需要がある。ただし全体的には ChatGPT が依然として最も人気の AI である。

- **ソース**: [TechCrunch Claude](https://techcrunch.com/2026/06/25/anthropics-claude-is-winning-over-paid-consumers-a-market-owned-by-chatgpt)
- **重要度**: 7/10
- **タグ**: pricing, cowork, 新機能

---

## 2026-04-03

### Claude Codeの首位陥落。CodexがシェアNo.1へ。 ～データで見る2026年3月のAI Codingの動向まとめ～

AI Coding.Infoの2026年3月データによると、Codex CLIがClaude Codeを抜いてシェア1位に。AI Coding Agent全体の利用率は8.7%（前月比1.4%増）と予測を超える急成長。言語別ではTypeScriptが1位、Rustが2位で頭抜けた成長を見せる。GPT-5.3-Codex-Sparkのリリース（2/12）を境にRust/C++での利用が急増した可能性。

- **ソース**: [Qiita claude](https://qiita.com/kotauchisunsun/items/ab78bb338500b4c71103)
- **重要度**: 6/10
- **タグ**: claude-code, performance, cowork

---

## 関連リンク

- [Claude Info トップ](../README.md)

---

## 更新履歴

| 日付 | 内容 |
|------|------|
| 2026-04-03 | 自動生成 |
